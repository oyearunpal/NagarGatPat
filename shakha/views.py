...
def nagar_about(request):
	...
	if today.day > 20:
		if today.month is 12 :
			birthday_next_10days = swaymsevak_list.filter(Q(spersonal__dob__day__gte=today.day,spersonal__dob__month=today.month)| Q(spersonal__dob__day__lte=31-today.day,spersonal__dob__month = 1) )
		else:
			birthday_next_10days = swaymsevak_list.filter(Q(spersonal__dob__day__gte=today.day,spersonal__dob__month=today.month)| Q(spersonal__dob__day__lte=31-today.day,spersonal__dob__month = today.month+1) )
	else:
		birthday_next_10days = swaymsevak_list.filter(spersonal__dob__day__range=[today.day,today.day+10],spersonal__dob__month=today.month)
...
def index(request, shakha_id):
	...
	shakha_toli = swaymsevak_list.exclude(sshakha__jimmedari='1').exclude(sshakha__jimmedari='0').exclude(sshakha__jimmedari='3').exclude(sshakha__jimmedari__isnull=True).exclude(sshakha__jimmedari__exact='')
	# Exclude result of blank or none society name
	list_society=Spersonal.objects.filter(name__shakha=shakha_id).exclude(society__isnull=True).exclude(society__exact='').values('society').distinct()
# [{'society': u'Jai Durga'}, {'society': u'Sainath Chal'} ]
	...
	value_ListSociety=[]
	for i in list_society:
		a=dict(society=str(i['society']),value=Spersonal.objects.filter(name__shakha=shakha_id).filter(society=str(i['society'])).count())
		value_ListSociety.append(a)
	donut_value=[]	
	for i in list_society:
		a=dict(label=str(i['society']),value=Spersonal.objects.filter(name__shakha=shakha_id).filter(society=str(i['society'])).count())
		donut_value.append(a)
...
def shakha_gat(request,shakha_id):
	...
	shakha = get_object_or_404(Shakha, pk=shakha_id)
	swaymsevak_gat = Sshakha.objects.filter(gat__in=shakha_gat).values('gat__name','name__fname','name__mname','name__lname','name__spersonal__contact','ganvesh_complete','name__id',)
